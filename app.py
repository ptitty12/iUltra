...
def build_vocab():
    ...
    for pat, _t, _a, _o in DRINK_PATTERNS:
        m = re.match(r"^\\b\((.*)\)s\?\\b$", pat.pattern, re.S)
        if not m:
            continue
...
WORD_QTY = {"one":1,"two":2,"three":3,"four":4,"five":5,"six":6,"couple":2,"few":3}
QTY_WORD_RE = re.compile(r"\b(" + "|".join(WORD_QTY) + r")\b", re.I)
QTY_DIGIT_RE = re.compile(r"\b([1-9]|1[0-2])\b(?!\s*(?:%|oz\b))", re.I)
QTY_TIMES_RE = re.compile(r"\bx\s*([1-9]|1[0-2])\b|\b([1-9]|1[0-2])\s*x\b", re.I)
# a "double" is two pours, but double ipa / triple sec / double black are names
MULTIPLIER_RE = re.compile(r"\b(double|triple)\b(?!\s*(?:sec|ipa|bock|black|india))", re.I)

def parse_qty(text, dtype):
    """how many of the thing were had. 1 unless the text says otherwise.

    Skipped entirely when the whole message is itself a drink name, so brands that
    read like counts — two robbers, four loko, 1800, 151 — stay one drink."""
    if text.strip().lower() in VOCAB_SET:
        return 1
    m = QTY_TIMES_RE.search(text)
    if m:
        return int(m.group(1) or m.group(2))
    m = QTY_WORD_RE.search(text)
    if m:
        return WORD_QTY[m.group(1).lower()]
    m = QTY_DIGIT_RE.search(text)
    if m:
        return int(m.group(1))
    if dtype in ("spirit shot", "cocktail") and MULTIPLIER_RE.search(text):
        return 2 if MULTIPLIER_RE.search(text).group(1).lower() == "double" else 3
    return 1
...
def parse_drink(text):
    ...
    # quantity scales the volume, so std drinks and bac both follow from it
    return dtype, abv, oz * parse_qty(text, dtype)
...
def calc_bac(drinks, now_ms=None):
    ...
    Walks the drinks in order and burns alcohol off between them, clamping at zero
    each step. Subtracting METABOLISM * (total elapsed) from the summed peak instead
    would keep eliminating through stretches where there was nothing left to
    eliminate, so an early drink banks negative alcohol against everything that
    comes after it — a beer at dinner would silently cancel most of a late night."""
    if now_ms is None:
        now_ms = int(datetime.now().timestamp() * 1000)
    if not drinks:
        return 0.0
    ordered = sorted(drinks, key=lambda d: d["ts"])
    bac = 0.0
    prev_ts = ordered[0]["ts"]
    for d in ordered:
        bac = max(0.0, bac - METABOLISM * (d["ts"] - prev_ts) / 3600000)
        bac += peak_bac([d])
        prev_ts = d["ts"]
    return max(0.0, bac - METABOLISM * (now_ms - prev_ts) / 3600000)
...
def list_sessions():
    ...
    for s in sessions:
        ...
        result.append({
            ...
            "bac": bac,
            "total_std": total_std,
            # the client burns this down from the moment it receives it, so the
            # header keeps ticking between fetches without re-running the model
